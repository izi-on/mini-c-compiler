...
public class Parser extends CompilerPass {
    ...
    private Decl parseDecl() {
        ...
        if (accept(Category.LSBR)) {
            ...
            error(Category.LSBR, Category.LPAR);
            nextToken();
            return new VarDecl(BaseType.UNKNOWN, id);
        }
    }
    ...
    private Decl parseFunDeclOrFunDef(Type type, String id) {
        ...
        if (accept(Category.LBRA)) {
            ...
            return new FunDef(type, id, params);
        } else if (accept(Category.SC)) {
            nextToken(); // consume ";"
            return new FunDecl(type, id, params, null); // function prototype (block is null)
        } else {
            error(Category.LBRA, Category.SC);
            nextToken();
            return new FunDecl(type, id, params, null);
        }
    }
